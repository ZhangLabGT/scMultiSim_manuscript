...
grn_out0 <- read.csv("~/vm_shared/out_mf.csv", sep = "\t", header = TRUE)[1:400,]
adj_list <- list()
...
tenx_sce <- as.SingleCellExperiment(mf)
...
sim_sergio1 <- function () {
  ...
  write(out_data, file = "figures/real_data/grn_sergio_merfish.txt")
  ...
  write(tf_data, file = "figures/real_data/tf_sergio_merfish.txt")
  id_map
}
...
stop()
sg_out_expr <- read.csv("~/gt/SERGIO/expr_merfish.csv", header = FALSE)
...
ggsave("figures/real_data/dist_merfish.pdf", p_dist, width = 20, height = 3.5)
