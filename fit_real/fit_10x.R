...
grn_out <- read.csv("~/vm_shared/out_10x.csv", sep = "\t", header = TRUE)[1:400,]
...
sim_scmultisim <- function () {
  master_tfs <- (function () {
    .find_parent <- function(tg, depth = 0) {
      tf <- grn_out[grn_out$target == tg, "TF"]
      if (length(tf) == 0) {
        return(tg)
      } else if (length(tf) == 1) {
        return(.find_parent(tf, depth + 1))
      } else {
        unique(unlist(sapply(tf, \(x) .find_parent(x, depth + 1), USE.NAMES = F)))
      }
    }
    master_tfs <- c()
  ...
  grn_out2 <- grn_out
  grn_out2[,"importance"] <- grn_out2[,"importance"] / sum(grn_out2[,"importance"])

  .find_eff <- function(tf, tg) {
    # print(sprintf("TF: %s, target: %s", tf, tg))
    rows <- which(grn_out2$TF == tf)
    res_eff <- 0
    for (i in rows) {
      t2 <- grn_out2[i, "target"]
      imp <- grn_out2[i, "importance"]
      if (length(t2) == 0) {
        next
      }
      if (t2 == tg) {
        ...
        res_eff <- res_eff + imp * .find_eff(t2, tg) * 100
      }
    }
    sum(res_eff)
  }

  for (target in unique(grn_out$target)) {
    for (tf in unique(master_tfs)) {
      eff <- .find_eff(tf, target)
      if (eff > 0) {
        grn_out_scm <- rbind(grn_out_scm,
                             data.frame(TF = tf, target = target, importance = eff))
      }
    }
  }

  grn_out_scm$importance <- grn_out_scm$importance * 100
  grn_out_scm$importance[grn_out_scm$importance < 1] <- 1

  myTree <- ape::read.tree(text = "(A:0.1,B:0.1,C:0.1,D:0.1);")
...
sg_out_expr <- read.csv("~/gt/SERGIO/expr_10x.csv", header = FALSE)
rownames(sg_out_expr) <- sergio_id_map[rownames(sg_out_expr)]
...
plot_cc_heatmap(res$counts, res$cell_meta$pop,
                filename = "figures/real_data/cc_10x_scmultisim_true.png")
